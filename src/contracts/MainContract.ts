...
export class MainContract implements Contract {
    ...
    async sendIncrementMessage(provider: ContractProvider, sender: Sender, value: bigint, incrementBy: number) {
        const msgBody = beginCell().storeUint(1, 32).storeUint(incrementBy,32).endCell();
        await provider.internal(sender, {
            value, sendMode: SendMode.PAY_GAS_SEPARATELY, body: msgBody
        });
    }
